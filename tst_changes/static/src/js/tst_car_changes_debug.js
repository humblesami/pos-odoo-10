odoo.define('pos_product_creation', function(require) {
    ...
    gui.define_popup({
        name: 'car_reading_widget',
        widget: carReadingWidget
    });
    ...
    var selectEmployeesWidget = PosBaseWidget.extend({
        ...
        show: function(options) {
            ...
            if (this.pos.table) {
                this.selected_employees = this.pos.tables_by_id[this.pos.table.id].selected_employees;
            } else {
                this.selected_employees = [];
            }
    ...
    gui.define_popup({
        name: 'select_employees_widget',
        widget: selectEmployeesWidget
    });
    ...
    gui.define_popup({
        name: 'car_list_create',
        widget: carListCreationWidget
    });
    ...
    PaymentScreenWidget.include({
        ...
        render_readingwidget: function() {
            ...
            var methods = $(QWeb.render('TstCarReadingWidget', {
                widget: this,
                currentCarIndex: current_car_index
            }));
    ...
    ScreenWidget.include({
        ...
        display_client_details: function(visibility, partner, clickpos) {
            ...
            contents.on('click', '.button.add-car', function() {
                self.showAddCarPopup(partner);
            });
    ...
    RestaurantFloor.TableWidget.include({
        render_flooar_additional: function() {
            var self = this;
            var methods = $(QWeb.render('SingleTableTSTData', {
                widget: this
            }));
            return methods;
        },
    ...
    var carCustomerSearchWidget = PosBaseWidget.extend({
        ...
        search_partner_table: function(e) {
            ...
            if (e.keyCode) {
                ...
                /*
                new Model('user.cars').call('search_string',[1, order_vals]).then(function(results){
                    self.isSearch = true;
                    self.searchResult = results;
                    var car_list_widget = newThisIs.renderCarsTableList('server',results);
                    if(car_list_widget != 'n') {
    ...
    gui.define_popup({
        name: 'car_customer_search_widget',
        widget: carCustomerSearchWidget
    });
    ...
    models.Order = models.Order.extend({
        add_product: function(product, options) {
            ...
            if (product.pos_discount_apply == 'apply_fix') {
                _super_Order.add_product.call(this, product, {
                    discount_fixed: product.product_disc
                });
            } else if (product.pos_discount_apply == 'apply_percentage') {
                _super_Order.add_product.call(this, product, {
                    discount: product.product_disc_percentage
                });
            } else {
    ...
    return {
        carReadingWidget: carReadingWidget,
